        ...
        module.exports = (plugin) => {
            plugin.controllers.auth.callback = async (ctx) => {
                ...
                if (provider === 'local') {
                    await validateCallbackBody(params, 'Custom error message');
                    ...
                    return ctx.send({
                        ...
                        refresh: issueRefreshToken({ id: user.id }), //블로그 글처럼 하면 쿠키에만 쌓이길래 내가 추가한 코드
                        user: await sanitizeUser(user, ctx),
                    });
                }
            ...
            plugin.controllers.auth['refreshToken'] = async (ctx) => {
                ...
                try {
                    const obj = await verifyRefreshToken(refresh);
